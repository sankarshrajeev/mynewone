...
juce::AudioProcessorValueTreeState::ParameterLayout TapSynthAudioProcessor::createParameters()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>("OSC1WAVETYPE", "OSC 1 Wave Type" ,juce::StringArray{"Sine","Saw","Saquare"}, 0)  );
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>("FILTERTYPE", "Filter Type" ,juce::StringArray{"Low Pass","High Pass","Bandpass"}, 0)  );
    
    params.push_back(std::make_unique<juce::AudioParameterFloat>("FILTERFREQ","Filter Freq",juce::NormalisableRange<float>{20.0f,20000.0f,0.6f},200.0f));
